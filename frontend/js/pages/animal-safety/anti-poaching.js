...
// ========== COMMUNITY ITEMS ANIMATION ==========
function initializeCommunityItems() {
...
function initializeCTAAnimations() {
    ...
    if (ctaSection) {
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ctaSection.style.background = 'linear-gradient(135deg, #22c55e, #dc2626)';
...
function handleResize() {
    ...
    if (width < 768) {
        ...
        document.querySelectorAll('.initiative-card, .law-card, .tech-card, .action-card').forEach(card => {
